 ...
 * V2 improvements:
 * - Lower ticket price (0.0001 ZETA)
 * - Configurable maximum participants per raffle
 */
contract ZetaRaffleV2 is Ownable, ReentrancyGuard, ZetaReceiver {
    using EnumerableMap for EnumerableMap.UintToAddressMap;
    using EnumerableMap for EnumerableMap.AddressToUintMap;

    // Constants
    uint256 public constant TICKET_PRICE = 1 * 10**14; // 0.0001 tokens (adjusted for decimals)
    ...
    struct RaffleInfo {
        ...
        uint256 maxParticipants; // NEW: Maximum number of participants (0 means unlimited)
    }
     ...
     * @param maxParticipants Maximum number of participants (0 means unlimited)
     */
    ...
    function buyTickets(
        ...
        if (raffle.maxParticipants > 0) {
            // Count current unique participants and check if adding a new one would exceed the limit
            uint256 participantCount = _countParticipants(raffleId);
            bool isNewParticipant = raffleParticipants[raffleId][msg.sender].ticketCount == 0;
    ...
    function onZetaMessage(
        ...
        if (raffle.maxParticipants > 0) {
            // Count current unique participants and check if adding a new one would exceed the limit
            uint256 participantCount = _countParticipants(raffleId);
            bool isNewParticipant = raffleParticipants[raffleId][participant].ticketCount == 0;
    ...
    /**
     * @dev Count unique participants in a raffle
     */
    function _countParticipants(uint256 raffleId) internal view returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < _raffleIdCounter; i++) {
            Participant storage p = raffleParticipants[raffleId][address(uint160(i))];
            if (p.ticketCount > 0) {
                count++;
            }
        }
